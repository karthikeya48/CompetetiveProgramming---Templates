...
signed main() {
    ...
    while(tt--) {
        ...
        auto update = [&](int index, int x) -> void {
        	a[index] = x;
        	int blockNumber = index / sq;
        	int blockStart = (blockNumber * sq);
        	int blockEnd = min(n, blockStart + sq);
        	blocks[blockNumber] = LLONG_MIN;
        	for(int i = blockStart; i < blockEnd; i++) {
        		blocks[blockNumber] = max(blocks[blockNumber], a[i]);
        	}
        };
